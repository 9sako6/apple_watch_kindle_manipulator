...
Apple WatchやiPadOSのメディア操作を、iPad Safari上のKindle for Webのページ送りへ変換するユーザースクリプトです。Apple Developer Programや独自のiOSアプリは必要ありません。
...
- Macまたは、このリポジトリから生成済みスクリプトをiPadへ渡す手段
